    ...
    public Orden saveOrden(Orden entity, List<OrdenHasEstado> ordenEstados) throws Exception {
        try {
            ...
            for (int i = 0; i < ordenEstados.size(); i++) {
                ...
                // ordenHasEstadoRepository.saveOrdenHasEstado(oe.get(i).getFechaAsignado(),
                // oe.get(i).getIdUsuario(),
                // oe.get(i).getEstadoIdEstado(),oe.get(i).getOrdenIdOrden());

                // ordenRepository.saveOrdenHasEstado(ordenEstados.get(i).getFechaAsignado(),
                // ordenEstados.get(i).getIdUsuario(),
                // ordenEstados.get(i).getEstadoIdEstado(),ordenEstados.get(i).getOrdenIdOrden());
